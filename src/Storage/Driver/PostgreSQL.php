...
class PostgreSQL
{
    ...
    public static function initializeStorage(): bool
    {
        ...
        try {
            ...
            if ($dsn === false || empty($dsn['host']) || empty($dsn['path'])) {
                throw new StorageException('Invalid DATABASE_URL value!');
            }
            ...
            $pdo_dsn = 'pgsql:host=' . $dsn['host'] . ';port=' . ($dsn['port'] ?? 5432) . ';dbname=' . ltrim($dsn['path'], '/');
            $sslmode = $query['sslmode'] ?? getenv('PGSSLMODE');
            if (empty($sslmode) && getenv('DYNO')) {
                // Heroku Postgres enforces TLS - default to a safe mode on dynos.
                $sslmode = 'require';
            }

            if (!empty($sslmode)) {
                $pdo_dsn .= ';sslmode=' . $sslmode;
            }

            self::$pdo = new PDO($pdo_dsn, isset($dsn['user']) ? rawurldecode($dsn['user']) : null, isset($dsn['pass']) ? rawurldecode($dsn['pass']) : null);
