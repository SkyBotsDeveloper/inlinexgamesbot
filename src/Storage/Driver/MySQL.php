    ...
    public static function initializeStorage($pdo = null): bool
    {
        ...
        if ($pdo === null) {
            try {
                ...
                self::$pdo = new PDO(
                    'mysql:host=' . $dsn['host'] . ';port=' . ($dsn['port'] ?? 3306) . ';dbname=' . ltrim($dsn['path'], '/') . ';charset=utf8mb4',
                    isset($dsn['user']) ? rawurldecode($dsn['user']) : null,
                    isset($dsn['pass']) ? rawurldecode($dsn['pass']) : null
                );
